    ...
    if check_only_save_mm_adapter_tunnable:
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            if current_folder.startswith("checkpoint-"):
                ...
                torch.save(weight_to_save, os.path.join(output_dir, f"mm_projector.bin")) # <- default when training adapter
        return
...
def preprocess_plain(
    ...
    for source in sources:
        ...
        conversation = source[0]["value"] + source[1]["value"] + "<|end_of_text|>"
...
#     conversations = []
#     for source in sources:
#         print("\n=== Debugging Source Data ===")
#         print(f"Original human input:\n{source[0]['value']}")
#         print(f"Original GPT output:\n{source[1]['value']}")
#         assert len(source) == 2
#         assert DEFAULT_IMAGE_TOKEN in source[0]["value"]
#         source[0]["value"] = DEFAULT_IMAGE_TOKEN
#         print(f"\nModified human input after replacing with IMAGE_TOKEN:\n{source[0]['value']}")
#         conversation = source[0]["value"] + source[1]["value"] + conversation_lib.default_conversation.sep
#         print(f"\nFull conversation:\n{conversation}")
#         conversations.append(conversation)
#     # tokenize conversations
...
#     print("\n=== Debugging Masking ===")
#     for target, source in zip(targets, sources):
#         print(f"\nTokenizing human input (IMAGE_TOKEN)...")
#         human_tokens = tokenizer_image_token(source[0]["value"], tokenizer)
#         tokenized_len = len(human_tokens)
#         print(f"Number of tokens in human input: {tokenized_len}")
#         target[:tokenized_len] = IGNORE_INDEX
#         print(f"\nMasking statistics:")
#         print(f"Total sequence length: {len(target)}")
#         print(f"Number of masked positions (IGNORE_INDEX): {(target == IGNORE_INDEX).sum().item()}")
#         print(f"Number of unmasked positions: {(target != IGNORE_INDEX).sum().item()}")
#         # Optionally print some tokens for verification
#         print("\nFirst few tokens of the sequence:")
#         for i in range(min(10, len(target))):
#             print(f"Position {i}: {'MASKED' if target[i] == IGNORE_INDEX else 'UNMASKED'} (token_id: {target[i]})")
#     return dict(input_ids=input_ids, labels=targets)
...
class LazySupervisedDataset(Dataset):
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        elif image_aspect_ratio == "pad": # <- default
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            sources = [sources[0]['conversations']]
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    # Validation 데이터 경로 설정 - train 데이터 경로에서 train을 val로 변경
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            or "llama" in model_args.model_name_or_path.lower() # <- default
            or "yi" in model_args.model_name_or_path.lower()
            or "nous-hermes" in model_args.model_name_or_path.lower()
            and "wizard-2" in model_args.model_name_or_path.lower()
        ):
            rank0_print("Initialize the model from LlavaLlamaForCausalLM")
...
################# qlora+deepspeed
#         compute_dtype = torch.float16 if training_args.fp16 else (torch.bfloat16 if training_args.bf16 else torch.float32)
#         quant_storage_dtype = compute_dtype  # 저장 dtype을 compute_dtype과 동일하게 설정
...
#         )
#         bnb_model_from_pretrained_args.update(dict(quantization_config=bnb_config,))

# ################# qlora+deepspeed

         ################# qlora+deepspeed 때문에 주석처리
    ...
    if model_args.version == "v0":
        ...
        if model_args.version in conversation_lib.conv_templates:
            conversation_lib.default_conversation = conversation_lib.conv_templates[model_args.version] # <- default
        else:
